...
async def send_daily_question(context):
    chat_id = os.environ["TELEGRAM_CHAT_ID"]
    try:
        student, topic, assignment, hint = get_random_assignment()
        logger.info(f"Генерирую вопрос: ученик={student}, тема={topic}, задание={assignment[:50]}")
...
def main():
    ...
    # 12:00 МСК = 09:00 UTC, 19:00 МСК = 16:00 UTC
    schedule = [
    ...
    async def job_wrapper():
        class FakeContext:
            bot = app.bot
        await send_daily_question(FakeContext())

    for hour, minute in schedule:
        scheduler.add_job(job_wrapper, trigger="cron", hour=hour, minute=minute)

    scheduler.start()
    times = ", ".join(f"{h:02d}:{m:02d} UTC" for h, m in schedule)
    logger.info(f"Бот запущен. Вопросы будут отправляться в {times} (12:00 и 19:00 МСК)")
    app.run_polling()
