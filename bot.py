...
def generate_question(student: str, topic: str, assignment: str, hint: str) -> dict:
    ...
    # Сначала просим модель вспомнить факты, потом составить вопрос на их основе
...
ШАГ 1 — напиши раздел ФАКТЫ: перечисли 3-5 точных фактов по теме "{assignment}" которые ты знаешь достоверно. Если не уверен в факте — не пиши его.

ШАГ 2 — напиши раздел JSON: составь вопрос и варианты ответа СТРОГО на основе фактов из шага 1.
...
- Все варианты одного типа (все имена, все номиналы, все города — не смешивай)
- Неправильные варианты правдоподобны но однозначно неверны
- Опирайся ТОЛЬКО на факты которые написал в шаге 1
