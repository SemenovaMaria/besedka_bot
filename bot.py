...
def generate_question(student: str, topic: str, assignment: str, hint: str) -> dict:
    ...
    prompt = f"""Ты составляешь вопрос для викторины по конкретному учебному материалу.

Ученик: {student}
Тема урока: {topic}
Конкретное задание ученика: {assignment}{hint_block}

Задача: составь вопрос, в котором правильный ответ — это именно "{assignment}".
Вопрос должен начинаться с обращения по имени "{student}".

Например: если задание "10 евро" и тема "евро" — спроси что-то вроде
"{student}, купюра какого номинала изображает романский архитектурный стиль?" (правильный ответ: 10 евро)
...
Правила:
- Правильный ответ ВСЕГДА равен заданию ученика: "{assignment}"
- Остальные 3 варианта — правдоподобные, но неверные (из той же темы)
- correct_index — индекс правильного ответа среди options (0–3), перемешивай позицию
- Все варианты одного типа (все номиналы, все имена, все города — не мешай разные категории)
- Язык: русский"""
